...
expectTrue('2.2 compactDock ise özet satırı render (verb + to chip\'leri + ayrıntılar toggle)',
  /compactDock && \(\s*<div className=\{`flex items-center gap-2 \$\{MAIL_TYPE\.t2\}`\}>[\s\S]{0,400}\{summaryVerb\}/.test(composer));
...
expectTrue('2.5 Ayrıntılar toggle özet satırında (aria-expanded={showAdvanced})',
  /compactDock && \(\s*<div className=\{`flex items-center gap-2 \$\{MAIL_TYPE\.t2\}`\}>[\s\S]{0,3000}aria-expanded=\{showAdvanced\}[\s\S]{0,300}ayrıntılar/.test(composer));
...
expectTrue('3.2 Uyarı özet satırı ALTINDA (gönderim engelini gizleme)',
  /compactDock && \(\s*<div className=\{`flex items-center gap-2 \$\{MAIL_TYPE\.t2\}`\}>[\s\S]{0,2000}compactDock && noAliasesConfigured/.test(composer));
