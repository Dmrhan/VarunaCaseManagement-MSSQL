...
const oneri = [];
for (let i = 1; i <= 7; i++) {
  const p = path.join(root, 'scripts', `eval3-part-${i}.json`);
...
let out = null;
for (const name of ['VK-yeniden-etiketleme.xlsx', 'VK-yeniden-etiketleme-v2.xlsx', 'VK-yeniden-etiketleme-v3.xlsx']) {
