...
// ── 11) resolvedFrom/resolvedTo → terminal statü guard'ı (P2 fix) ──
// reopen edilmiş (şu an açık) vakada eski resolvedAt korunuyor
// (caseRepository.js) — bu filtre statüsüz sızmamalı.
console.log('\n── 11) Resolved range — terminal statü guard\'ı ───────────');
{
  const { where } = buildReportWhere({ resolvedFrom: '2026-01-01' }, ALLOWED);
  expect('11.1 statü seçilmediyse otomatik terminale kısıtlanır',
    where.status, { in: ['Cozuldu', 'IptalEdildi'] });
}
{
  // Kullanıcı 'Açık' seçip aynı anda resolvedFrom verirse — çelişkili
  // kombinasyon, kesişim BOŞ olmalı (0 satır, sızıntı değil).
  const { where } = buildReportWhere({ statuses: 'Açık', resolvedFrom: '2026-01-01' }, ALLOWED);
  expect('11.2 çelişkili statü+resolvedFrom → boş kesişim (0 satır)',
    where.status, { in: [] });
}
{
  // Kullanıcı 'Çözüldü,İptalEdildi,Açık' seçip resolvedTo verirse —
  // kesişim yalnız terminal olanları bırakmalı.
  const { where } = buildReportWhere(
    { statuses: 'Çözüldü,İptalEdildi,Açık', resolvedTo: '2026-06-18' },
    ALLOWED,
  );
  expect('11.3 karışık statü seçimi + resolvedTo → yalnız terminal kalır',
    where.status, { in: ['Cozuldu', 'IptalEdildi'] });
}
{
  // resolvedFrom/To YOKSA statü filtresi olduğu gibi kalmalı (regression).
  const { where } = buildReportWhere({ statuses: 'Açık' }, ALLOWED);
  expect('11.4 resolvedFrom/To yokken statü filtresi dokunulmamış',
    where.status, { in: ['Acik'] });
}
