...
│   tüm roller için TEK giriş noktası
│   tıklayınca
│   └─ Drawer (right side)         ← Phase 2A, evrensel
│       clamp(420px, 34vw, 640px) genişlik; gerçek yan panel, dropdown DEĞİL
│       4 sekme: İşler / Bildirimler / Ertelenen / Tamamlanan
│       her satır = inline mini-aksiyonlar
│       footer link: Phase 2A'da YOK (§18.H kararına bağlı)
│
├─ MyHome /                        ← mevcut + küçük rötüş
│   Erişimi olan roller için GENİŞ operasyonel günlük çalışma alanı
│   "Onayımda Bekleyenler" panel
│   "Aksiyonlarım" linki + sayaç (drawer'ı açar)
│   "RUNA Önerileri" (eski Önerilen Aksiyonlar) — inbox'tan AYRI
│
│   ⚠ My Home erişimi olmayan roller için Phase 2A'da
│     drawer'ın dışında rakip bir geniş yüzey AÇILMAZ.
│     Wide surface kararı §18.H'de Phase 3 öncesi verilir.
│
...
│
├─ Full-page Inbox /inbox          ← Phase 2A'da YOK
│   §18.H'deki "wide surface model" kararından sonra Phase 3'te
│   inşa edilir (varsayılan değil — koşullu). Tasarım taslağı §7.C.
│
└─ Admin
...
### 4.D. Product surface model — Phase 2A'da kullanıcı deneyimi

Phase 2A'da kullanıcının inbox deneyimi role bağımlı olarak farklılaşır. Üç kategori:

**Herkes (tüm roller — Agent / Supervisor / CSM / Backoffice / Admin / SystemAdmin):**
- Sağ-üst tek "Aksiyonlarım" bell ve drawer.
- Bu **evrensel quick inbox** — her rol için, ekranın neresinde olursa olsun aynı erişim.
- Drawer = §7.B'deki gerçek operasyonel yan panel (clamp 420-640 px, dropdown değil).

**My Home erişimi olan roller (örn. Agent, Supervisor):**
- Drawer'a **ek olarak** My Home'da geniş günlük operasyonel pano.
- `Onayımda Bekleyenler` widget'ı (Phase 1'den canlı).
- MyHome My Home erişimi olan rollerin **genişletilmiş çalışma alanı**dır; inbox'ın yerini almaz, tamamlar.

**My Home erişimi olmayan roller:**
- Phase 2A'da yalnız Aksiyonlarım drawer'ı kullanır.
- **Rakip bir geniş yüzey AÇILMAZ.** Phase 2A scope'unda `/inbox` yok, ayrı admin panel yok, ek dashboard sayfası yok.
- Bu roller için geniş wide-surface ihtiyacı varsa, çözüm önce **My Home erişiminin onlara da açılması** veya **ayrı `/inbox` inşası** kararı verilmesidir. Bu karar §18.H'de Phase 3 öncesi netleştirilir.

Bu modelin tek kuralı: **rekabet eden bildirim/iş yüzeyi yaratma.** Phase 2A boyunca tüm aksiyon ve bilgilendirme sinyalleri tek bir zile (sağ-üst Aksiyonlarım) toplanır. Eski sağdaki bahsetme zili pilot başında **hemen gizlenir** (§5.B, §19.A); kullanıcı iki zil arasında gidip gelmez.
...
- Footer link: Phase 2A'da YOK. "Tümünü gör → /inbox" linki ancak §18.H'deki ürün kararı verildikten sonra (Phase 3+) anlam kazanır. Aksi halde tıklanınca gidecek yer yok.

#### 7.B.1 Drawer boyutu, yerleşimi ve davranışı

Aksiyonlarım drawer'ı **küçük bir dropdown değil**, ekranın yaklaşık üçte birini kaplayan **gerçek operasyonel yan panel**dir. Operatör bir kola yaslandığında ne kadar iş olduğunu görüp aksiyon alabilmeli.
...
Hedefler (acceptance):
- 1440 px desktop'ta drawer ≈ **480–520 px** (1440 × 0.34 = 489 px).
- 1920 px desktop'ta drawer ≈ **600–640 px** (1920 × 0.34 = 652 px, üst sınır 640 px).
- 1280 px laptop'ta drawer ≈ **435–450 px**.
- Hiçbir desktop'ta drawer 420 px'in altına düşmez; 640 px'i geçmez.

**Tablet / dar viewport (md/lg arası):**
...
Tam ekran sheet — `inset: 0`, backdrop opak, drawer scroll'lu içerik.

**Yerleşim sözleşmesi:**

| Kural | Davranış |
|---|---|
| Sağ-üst sabit overlay | `position: fixed; right: 0; top: 0; bottom: 0;` — sayfa scroll'undan etkilenmez. |
| Main content **kaymaz** | Drawer açılınca ana içerik genişlik değiştirmez; backdrop opsiyonel (Phase 2A'da hafif `bg-slate-900/30` mobile-only kalır). Desktop'ta backdrop **yok**; main içerik visible kalır ama drawer üstünde durur. |
| Header sticky | Drawer içinde başlık + close butonu üstte sabit; scroll edilmez. |
| Tabs sticky | Header'ın hemen altındaki sekmeler de sabit; satır listesi kaydırılırken sekmeler kalır. |
| İçerik internal scroll | Yalnızca satır listesi alanı kaydırılır; sayfa scroll'una taşmaz. |
| Action butonları wrap olmaz | Satır altındaki buton şeridi en az 420 px drawer içinde yatay sığar; sığmazsa overflow `...` menüsü (Phase 2B+'ta). Phase 2A'da sığması garanti edilir (mevcut buton sayısı kind başına ≤5). |
| Açılış / kapanış stabil | `transform` veya `opacity` transition; layout shift YOK. Açılma 150-200ms, kapanma 100-150ms. |
| Z-index | Drawer header ve modal'lardan altta (`z-50`); diğer modal/alert tepesinde durmaz. |

**Acceptance kriterleri (drawer UX):**

| # | Kontrol | Beklenen |
|---|---|---|
| DR-1 | 1440 px desktop'ta drawer genişliği | 480–520 px |
| DR-2 | 1920 px desktop'ta drawer genişliği | 600–640 px |
| DR-3 | Mobile (≤640 px) drawer | Tam ekran (`width: 100vw`) |
| DR-4 | Drawer açıldığında main content | Asla genişlik / pozisyon değişmez |
| DR-5 | Sticky header + tabs | Liste kaydırıldığında üstte sabit kalır |
| DR-6 | Action butonları | Tek satır içinde sığar; yatay overflow yok |
| DR-7 | Drawer transition | <250ms, layout shift yok |
| DR-8 | İç içe (nested) help drawer | YOK (Phase 1 hotfix kuralı) |
| DR-9 | Drawer kapatma | Backdrop (mobile), ESC, `×` butonu — üçü de çalışır |
| DR-10 | Çoklu sekme/pencere | Bir pencerede drawer açıkken diğer pencerede açılma DOM/state çakışması yapmaz |

### 7.C. Wide / full-page surface — Phase 3 kararına bağlı

Phase 2A'da **tam sayfa /inbox YOK** (§18.H'deki refined karara bakınız). Aşağıdaki üçlü kolon tasarımı, ürün §18.H'de "wide surface model" finalize edildikten sonra **Phase 3'te** uygulanacak referans bir taslaktır. Phase 2A planı bu sayfaya bağımlı değildir.
...
Tasarım referansları (Phase 3'te uygulanacaksa):
- Üçlü kolon Phase 3'e ait — Phase 1/2/2A/2B/2C'de drawer yeter.
- Phase 3 build kararı **§18.H'deki "wide surface model" ürün kararına bağlıdır** — kendiliğinden inşa edilmez.
...
### 18.H. Full-page / wide surface — refined Phase 2A decision

**KARAR (verildi):** Phase 2A'da **ayrı `/inbox` rotası açılmayacak.**

**Gerekçe:** Phase 2A scope'u darca tanımlı — tek bell + drawer + mention migration + badge güveni + demo seed. Tam sayfa wide surface farklı bir ürün kararı gerektiriyor: çünkü **Benim Sayfam (My Home) erişimi role bağımlıdır**, ve "tam sayfa inbox kim için, nerede" sorusunun cevabı My Home erişim modeli netleşmeden verilemez.

**Mevcut model (Phase 2A için):**

| Surface | Kim için | Phase 2A davranışı |
|---|---|---|
| **Aksiyonlarım drawer** | Tüm roller — evrensel quick inbox | Sağ-üst tek zilden açılır; her rol için aynı çalışır |
| **My Home** | My Home erişimi olan roller (Agent, Supervisor, vb.) | Daha geniş günlük iş paneli + `Onayımda Bekleyenler` widget'ı |
| **My Home erişimi olmayan roller** | Bu roller için ek geniş yüzey | Phase 2A'da sadece **drawer**; başka rakip yüzey açılmaz |

**Phase 3 öncesi karara bağlanması gereken sorular:**

1. **My Home tüm operasyonel rollere açılacak mı?** (CSM, Backoffice, Admin için My Home erişimi şu an net değil — açılırsa ek wide surface ihtiyacı düşer.)
2. **Yoksa ayrı bir `/inbox` sayfası mı inşa edilecek?** (Wide surface universal hâle gelir; My Home erişiminden bağımsız çalışır.)
3. **Admin / SystemAdmin için inbox nerede yaşar?** My Home içinde mi, ayrı bir admin yüzeyinde mi, yoksa drawer mı yeterli?

**Phase 2A implication:**

Phase 3'ün kapısı **"varsayılan olarak `/inbox` inşa et"** değildir. Phase 3'ün ön koşulu önce yukarıdaki 3 sorunun **yazılı kararı**dır; sonra wide surface tasarımı buna göre derive edilir. Drawer Phase 2A/B/C'de yeterlidir; wide surface erken inşa edilirse yarım hisseder (Phase 1 hotfix dersi).

**Decision table satırı:**

| Konu | Karar |
|---|---|
| **Full-page / wide surface** | Phase 2A: **`/inbox` YOK.** Drawer tüm roller için. My Home erişimi olan roller için ek olarak My Home widget. My Home erişimi olmayan roller için Phase 2A'da **drawer-only**; ek sayfa AÇILMAZ. Phase 3 öncesi 3 soru cevaplanır (My Home erişim modeli + ayrı /inbox kararı + Admin/SystemAdmin inbox yüzeyi). |
...
| 2026-05-26 | v0.4 | Phase 2A product decisions: §18.H refined ("no `/inbox` in Phase 2A"; wide surface role-dependent My Home erişim kararına bağlı, Phase 3 öncesi 3 soru cevaplanır); §7.B drawer UX sizing rules (clamp 420-640px, gerçek operasyonel yan panel, dropdown değil, layout shift yok, 10 acceptance kriteri DR-1..DR-10); §7.C softened (full-page Phase 3 referans-only, decision-pending); §4.A surface diagram refined (My Home erişimi olmayan roller için rakip wide surface açılmaz); §4.D Product Surface Model eklendi (herkes / My Home gören / My Home görmeyen roller); §D.1 coverage row #5 güncellendi. |
...
| 5 | **Drawer vs full-page bölünmesi** — drawer hızlı aksiyon; full-page arama / filtre / bulk / geçmiş. | §4.A (refined diagram), §4.D (Phase 2A surface model), §7.B (drawer sizing 420-640 px), §7.C (full-page Phase 3 referansı), §13.A, §18.H (refined "no /inbox in Phase 2A" decision; wide surface kararı Phase 3 öncesi gerekli) | ✓ tam — Phase 2A drawer-only; wide surface §18.H kararına bağlı |
