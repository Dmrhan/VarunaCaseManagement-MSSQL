...
// WR-ACTION-CENTER Phase 1 — operator-facing help mounted in
// ActionCenterDrawer header. Sibling of the registry topic
// `action-center` in src/help/helpRegistry.ts (registry entry is used by
// smoke-help-content for keyword/banlist enforcement; this constant is
// what the drawer actually renders).
export const ACTION_CENTER_HELP: HelpContent = {
  title: 'Eylem Merkezi — Operatör Rehberi',
  sections: [
    {
      heading: 'Eylem Merkezi nedir?',
      content:
        'Üst çubuktaki ListChecks ikonu Eylem Merkezi zilidir. Bir vakaya senin karar vermen veya müdahalen gerekiyorsa ya da seninle ilgili bir sonuç ortaya çıktıysa o satır buraya düşer. Tıklayınca sağdan açılan panelde tüm açık iş satırların listelenir.',
      tip:
        'Soldaki yeni ikon Eylem Merkezi, sağdaki çan ise eski bahsetme zili. İkisi yan yana çalışır ve birbirinin yerini almaz.',
    },
    {
      heading: 'İki sayaç: Eylem Bekleyen ve Bildirimler (FYI)',
      content:
        'Zilin üstündeki kırmızı rakam senden somut bir aksiyon bekleyen satır sayısıdır (Onayla, Reddet, revize et). Gri rakam ise yalnız bilgilendirme niteliğindeki satır sayısıdır — aksiyon zorunlu değildir ama farkında olman istenir. Drawer sekmeleri aynı ayrımı yapar: Eylem Bekleyen / Bildirimler / Ertelenen / Yapıldı.',
    },
    {
      heading: 'Hangi tür satırları görürsün?',
      content:
        'Bu sürümde üç tür satır vardır:',
      example: `approval_pending
   → Bir vakanın çözüm onayı sana yönlendirildi.
     Satırın üzerinde Onayla / Reddet butonları çıkar.

approval_decided
   → Senin gönderdiğin bir çözüm onayı sonuçlandı
     (Onaylandı veya Reddedildi). Bilgilendirme;
     "Okundu" ile arşivlenir.

case_returned_to_assignee
   → Çözüm önerin reddedildi. Vakayı aç, revize et,
     yeniden çözüm onayına gönder, ardından satırı
     "Tamamlandı" ile kapat.`,
    },
    {
      heading: 'Bir satırda hangi eylemleri yapabilirsin?',
      content:
        'Her satırın üzerinde duruma göre değişen küçük butonlar vardır:',
      example: `Vakayı Aç     → Vaka detayına geçer; satır
                  otomatik "çalışıyor" durumuna döner.

Onayla / Reddet
              → Sadece sana düşen çözüm onayı
                  satırlarında görünür. Mevcut onay
                  endpointlerini çağırır.

Tamamlandı / Okundu
              → İşin sende bitti; satır arşivlenir.

Ertele        → 1 saat / yarın 09:00 / pazartesi 09:00
                  presetleriyle satırı geçici olarak
                  saklar; vakti gelince otomatik döner.

Yok Say       → Sana düşmediğini düşündüğün satırı
                  opsiyonel bir not bırakarak kapatır.`,
    },
    {
      heading: 'Bir satır niçin senin önünde çıktı?',
      content:
        'Satırın altındaki küçük "Çünkü..." cümlesi açıklar: çözüm onayı politikasının onaylayıcı kapsamında olmandan (Takım Lideri / Süpervizör / Admin rolü), vakanın halen sana atanmış olmasından veya senin göndericisi olduğun bir çözüm onayının sonuçlanmasından olabilir. Açıklama cümlesi hiçbir satırda boş bırakılmaz.',
    },
    {
      heading: 'Eski bahsetme zili ile farkı',
      content:
        'Sağdaki çan (bahsetme zili) hâlâ vaka yorumlarındaki @bahsetmeleri ve eski bildirim akışını gösterir. Eylem Merkezi (soldaki yeni ikon) yalnızca somut aksiyon ve karar satırlarını taşır; bahsetmeler buraya akmaz. Eski zilin davranışı değişmedi.',
      warning:
        'İki zilin sayaçları birbirini tekrar etmez. Bahsetme bildirimi için sağdaki zile, aksiyon bekleyen satırlar için soldaki Eylem Merkezi ziline bakın.',
    },
  ],
};
