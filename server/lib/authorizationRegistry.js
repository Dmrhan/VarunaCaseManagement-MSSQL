...
export const RESOURCE_REGISTRY = Object.freeze([
  ...
  { key: 'account', label: 'Müşteri', category: 'customer360', actions: ['create', 'read', 'update', 'delete'], currentEnforcement: 'server/routes/accounts.js role gates + repository scope + deny-only resource policy' },
  { key: 'account.contact', label: 'Müşteri Kontağı', category: 'customer360', actions: ['create', 'read', 'update', 'delete'], currentEnforcement: 'server/routes/accounts.js contact mutations + deny-only resource policy' },
  { key: 'account.project', label: 'Müşteri Projesi', category: 'customer360', actions: ['create', 'read', 'update', 'delete'], currentEnforcement: 'server/routes/accounts.js project mutations + deny-only resource policy' },
  { key: 'report.caseStudio', label: 'Vaka Rapor Stüdyosu', category: 'reporting', actions: ['read', 'export'], currentEnforcement: 'server/routes/reports.js role + column gates + deny-only resource policy' },
  { key: 'report.view', label: 'Kayıtlı Rapor Görünümü', category: 'reporting', actions: ['create', 'read', 'update', 'delete'], currentEnforcement: 'server/routes/reportViews.js owner/shared gates + deny-only resource policy' },
